...
import id.andtree.shared.reflection.ReflectionUtils;
...
import id.andtree.shared.termux.TermuxConstants.TERMUX_APP;
...
public class TermuxUtils {
     ...
     *
     * If your third-party app is targeting sdk `30` (android `11`), then it needs to add `id.andtree`
     * package to the `queries` element or request `QUERY_ALL_PACKAGES` permission in its
     * `AndroidManifest.xml`. Otherwise it will get `PackageSetting{...... id.andtree/......} BLOCKED`
     ...
     * <manifest
     *     <queries>
     *         <package android:name="id.andtree" />
     *    </queries>
     * </manifest>
     * }
     *
    ...
    public static String geAPTInfoMarkdownString(@NonNull final Context context) {
        ...
        InputStream inputStream = context.getResources().openRawResource(id.andtree.shared.R.raw.apt_info_script);
        try {
