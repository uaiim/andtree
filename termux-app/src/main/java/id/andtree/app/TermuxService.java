...
import id.andtree.R;
import id.andtree.app.event.SystemEventReceiver;
import id.andtree.app.terminal.TermuxTerminalSessionActivityClient;
import id.andtree.app.terminal.TermuxTerminalSessionServiceClient;
import id.andtree.shared.termux.plugins.TermuxPluginUtils;
import id.andtree.shared.data.IntentUtils;
import id.andtree.shared.net.uri.UriUtils;
import id.andtree.shared.errors.Errno;
import id.andtree.shared.shell.ShellUtils;
import id.andtree.shared.shell.command.runner.app.AppShell;
import id.andtree.shared.termux.settings.properties.TermuxAppSharedProperties;
import id.andtree.shared.termux.shell.command.environment.TermuxShellEnvironment;
import id.andtree.shared.termux.shell.TermuxShellUtils;
...
import id.andtree.shared.termux.TermuxConstants.TERMUX_APP.TERMUX_SERVICE;
import id.andtree.shared.termux.settings.preferences.TermuxAppSharedPreferences;
import id.andtree.shared.termux.shell.TermuxShellManager;
...
import id.andtree.shared.notification.NotificationUtils;
...
import id.andtree.shared.shell.command.ExecutionCommand.ShellCreateMode;
import id.andtree.terminal.TerminalEmulator;
    ...
    private synchronized void killAllTermuxExecutionCommands() {
        ...
        for (int i = 0; i < pendingPluginExecutionCommands.size(); i++) {
            ...
            if (!executionCommand.shouldNotProcessResults() && executionCommand.isPluginExecutionCommandWithPendingResult()) {
                if (executionCommand.setStateFailed(Errno.ERRNO_CANCELLED.getCode(), this.getString(id.andtree.shared.R.string.error_execution_cancelled))) {
